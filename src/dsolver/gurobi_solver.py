...
# Extra: SciPy MILP interface for Gurobi-free solve option
import numpy as np
try:
...
def solve_fixed_k_ilp(
    ...
    if milp is None or LinearConstraint is None or Bounds is None:
        raise RuntimeError(
            "SciPy MILP solver not available. Please install scipy>=1.11 to use solve_fixed_k_ilp_free()."
        )
    ...
    for i, d in enumerate(devs):
        ...
        if not (has_cuda or has_metal):
            ...
            if has_metal:
                head = 1.0 if d.is_head else 0.0
                caps.append(max(0, float(d.d_avail_metal) - float(d.c_gpu) - model.b_out * head))
            Nmax[i] = max(0, int(min(caps)))

    for y_pat in patterns:
        # Variables: x = [w | n] of length 2M
        def idx_w(i): return i
        def idx_n(i): return M + i
        Nvars = 2 * M
        ...
        # (3) RAM constraints per case with fixed y
        def bcio(i: int) -> float:
            return _b_cio(devs[i], model)
        ...
        # Objective for fixed y: k*(a^T w + b^T n + overload_terms(y))
    ...
    # Mirror Gurobi's helpful debug printout
